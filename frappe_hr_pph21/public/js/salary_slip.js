...
(() => {
    ...
    function render(data, {compact = false, dirty = false} = {}) {
        ...
        const postings = Array.isArray(data.noncash_accounting) ? data.noncash_accounting.filter(object) : [];
        const noncashJournal = postings.length ? `<h4>Jurnal noncash / BPJS dalam payroll</h4><p class="pph21-muted">Pasangan utang dibuat otomatis sebesar penghasilan noncash. Debit beban dan kredit utang tidak mengubah take-home pay; pasangan utang bukan pengurang pajak.</p><div class="pph21-scroll"><table class="pph21-components"><thead><tr><th>Komponen sumber</th><th>Debit beban</th><th>Kredit utang</th><th class="pph21-amount">Nominal</th></tr></thead><tbody>${postings.map(p => `<tr><td>${escape(p.component)}</td><td>${escape(p.expense_account)}</td><td>${escape(p.payable_account)}</td><td class="pph21-amount">${escape(money(p.amount))}</td></tr>`).join("")}</tbody></table></div>` : "";
        ...
        return `${css}<article class="pph21-sheet">${notice}${header}${cards}
            ...
            ${noncashJournal}<h4>Akumulasi sebelum masa ini</h4><p class="pph21-muted">Sudah termasuk saldo awal dan slip sebelumnya; jangan dijumlahkan lagi dengan saldo awal.</p>${table([row("Bruto pajak sebelumnya", money(data.prior_gross)),row("Pengurang tahunan sebelumnya", money(data.prior_deductions)),row("PPh21 neto sebelumnya", money(data.prior_tax))])}
            ...
            <dl class="pph21-grid">${meta("Akun beban tunjangan", accounts.allowance)}${meta("Akun utang PPh21", accounts.tax)}${meta("Akun pengembalian PPh21", accounts.refund)}${meta("Versi aturan / app", `${data.rule_version ?? "—"} / ${data.app_version ?? "—"}`)}</dl></details>
